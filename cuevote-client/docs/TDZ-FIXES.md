...
3. **Risky patterns**  
   ...
   - **Large data modules** (e.g. `legalContent`, and the per-language locale files under `contexts/translations/locales/`) loaded at startup — their export can be read before the module has finished running.
...
| **LanguageContext** | Statically imports only `translations/index.js`, which holds lazy loaders and no data. Every locale is still fetched with `import()` at runtime and stored in state, so no bulk of strings is ever evaluated at entry init. |
| **main.jsx** | App and LegalPage routes are `React.lazy()` so their chunks (and deps) don’t run at initial load. |
| **vite.config.js** | `manualChunks` for `legalContent` and a dedicated `contexts` chunk for ConsentContext.jsx + LanguageContext.jsx so they always run before the App chunk (avoids TDZ e.g. 'ce'). `sourcemap: true` so production errors point to real files. Translations need no rule: one module per language means Rollup already emits one chunk each. |
